...
static void event_wifi_connect_cancel(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_CLICKED)
    {
        ...
        {
            lv_obj_del_async(ui_wifi_connecting);
...
/***************** OpenAI *****************/
// lv_obj_t *msgbox = lv_msgbox_create(lv_scr_act(), "OpenAI_msgbox", message, NULL, true);
typedef struct
{
    bool WIFI_CONNECTED;
    bool OPENAI_SAVEDKEY;
} state_t;
static state_t openai_run_state = {
    .WIFI_CONNECTED = false,
    .OPENAI_SAVEDKEY = false,
};
...
void openai_show_msgbox(const char *message)
{
    ...
    lv_obj_set_style_text_color(msg, lv_color_hex(0x000000), LV_PART_MAIN | LV_STATE_DEFAULT);

    lv_obj_set_style_bg_color(openai_msg_tip, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_color(openai_msg_tip, lv_color_hex(0x000000), LV_PART_MAIN | LV_STATE_DEFAULT);
}

bool openai_is_ready(void)
{
    if (openai_run_state.OPENAI_SAVEDKEY == false)
    {
        openai_show_msgbox("\r\nPlease set the OpenAI API key first!");
        return false;
    }
    else if (openai_run_state.WIFI_CONNECTED == false)
    {
...
void ui_event_set_api_key_btn(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t *target = lv_event_get_target(e);
    ...
    {
        _ui_screen_change(ui_screen_openai_key, LV_SCR_LOAD_ANIM_OVER_BOTTOM, 200, 0);
    }
}

void ui_event_chatgpt_btn(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t *target = lv_event_get_target(e);
    lv_obj_t *cur_screen = lv_scr_act();
...
void ui_event_dalle_btn(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t *target = lv_event_get_target(e);
    lv_obj_t *cur_screen = lv_scr_act();
    if (event_code == LV_EVENT_CLICKED && cur_screen == ui_screen_openai)
    {
        if (openai_is_ready())
        {
            _ui_screen_change(ui_screen_dalle_1, LV_SCR_LOAD_ANIM_OVER_BOTTOM, 200, 0);
        }
    }
}

void ui_event_api_key_back_btn(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t *target = lv_event_get_target(e);
...
void ui_event_text_edit_gpt_request(lv_event_t *e)
{
    ...
    if (event_code == LV_EVENT_CLICKED)
    {
        lv_keyboard_set_textarea(ui_Keyboard_chatgpt, ui_text_edit_gpt_request);
    }
    if (event_code == LV_EVENT_CLICKED)
    {
        _ui_flag_modify(ui_Keyboard_chatgpt, LV_OBJ_FLAG_HIDDEN, _UI_MODIFY_FLAG_REMOVE);
    }
}

void ui_event_dalle_text(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t *target = lv_event_get_target(e);
    if (event_code == LV_EVENT_CLICKED)
    {
        lv_keyboard_set_textarea(ui_Keyboard_dalle, ui_dalle_text);
    }
    if (event_code == LV_EVENT_CLICKED)
    {
        _ui_flag_modify(ui_Keyboard_dalle, LV_OBJ_FLAG_HIDDEN, _UI_MODIFY_FLAG_REMOVE);
    }
}

void ui_event_chatgpt2_back_btn(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t *target = lv_event_get_target(e);
    if (event_code == LV_EVENT_CLICKED)
    {
        _ui_screen_change(ui_screen_chartgpt_1, LV_SCR_LOAD_ANIM_OVER_TOP, 200, 0);
    }
}

void ui_event_screen_openai(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t *target = lv_event_get_target(e);
...
static lv_obj_t *ui_chatgpt_wait;

lv_obj_t *ui_chatgpt_wait_create(lv_obj_t *parent)
{
...
static lv_obj_t *ui_request_wait;
static lv_obj_t *ui_request_state_bar;
static lv_obj_t *ui_request_state_msg;

lv_obj_t *ui_openai_request_wait_create(lv_obj_t *parent)
{
    lv_obj_t *obj = lv_obj_create(parent);
    ...
    ui_request_state_bar = lv_bar_create(obj);
    lv_obj_remove_style_all(ui_request_state_bar); /*To have a clean start*/
    lv_obj_add_style(ui_request_state_bar, &style_bg, 0);
    lv_obj_add_style(ui_request_state_bar, &style_indic, LV_PART_INDICATOR);

    lv_obj_set_size(ui_request_state_bar, 300, 20);
    lv_obj_center(ui_request_state_bar);
    lv_obj_set_y(ui_request_state_bar, -30);
    lv_bar_set_value(ui_request_state_bar, 0, LV_ANIM_ON);

    ui_request_state_msg = lv_label_create(obj);
    lv_obj_center(ui_request_state_msg);
    lv_obj_set_y(ui_request_state_msg, 30);
    lv_obj_set_style_text_font(ui_request_state_msg, &ui_font_font0, LV_PART_MAIN | LV_STATE_DEFAULT);
    // lv_label_set_text(ui_request_state_msg, "Request...");

    return obj;
}

void ui_event_generate_answer_btn(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t *target = lv_event_get_target(e);
    if (event_code == LV_EVENT_CLICKED)
    {
        struct view_data_openai_request req;
        const char *question = lv_textarea_get_text(ui_text_edit_gpt_request);

        strncpy(req.question, question, sizeof(req.question));
        esp_event_post_to(view_event_handle, VIEW_EVENT_BASE, VIEW_EVENT_CHATGPT_REQUEST, &req, sizeof(req), portMAX_DELAY);
...
void ui_event_regennerate_btn(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t *target = lv_event_get_target(e);
    if (event_code == LV_EVENT_CLICKED)
...
void ui_event_dall_supriseme(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t *target = lv_event_get_target(e);
    if (event_code == LV_EVENT_CLICKED)
    {
        struct view_data_openai_request req;
        const char *question = lv_textarea_get_text(ui_dalle_text);

        strncpy(req.question, question, sizeof(req.question));
        esp_event_post_to(view_event_handle, VIEW_EVENT_BASE, VIEW_EVENT_DALLE_REQUEST, &req, sizeof(req), portMAX_DELAY);

        if (ui_request_wait == NULL)
        {
            ui_request_wait = ui_openai_request_wait_create(ui_screen_dalle_2);
        }
        _ui_screen_change(ui_screen_dalle_2, LV_SCR_LOAD_ANIM_OVER_TOP, 200, 0);
    }
}

void ui_event_dalle_regennerate(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t *target = lv_event_get_target(e);
    if (event_code == LV_EVENT_CLICKED)
...
void ui_event_dalle2_back(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t *target = lv_event_get_target(e);
    if (event_code == LV_EVENT_CLICKED)
    {
        _ui_screen_change(ui_screen_dalle_1, LV_SCR_LOAD_ANIM_OVER_TOP, 200, 0);
    }
}

void ui_event_keyboard_hide(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t *target = lv_event_get_target(e);
...
static lv_img_dsc_t img_png = {
    0,
};

static bool image_st = 0; // 0: zoom out
void ui_event_dall_image(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t *target = lv_event_get_target(e);
    if (event_code == LV_EVENT_CLICKED)
    {
        ...
        if (image_st)
        {
            lv_img_set_zoom(ui_dall_image, 256); // normal
...
void openai_event_init(void)
{
    ...
    lv_obj_add_event_cb(ui_screen_openai, ui_event_screen_openai, LV_EVENT_ALL, NULL);
    lv_obj_add_event_cb(ui_set_api_key_btn, ui_event_set_api_key_btn, LV_EVENT_ALL, NULL);
    lv_obj_add_event_cb(ui_chatgpt_btn, ui_event_chatgpt_btn, LV_EVENT_ALL, NULL);
    lv_obj_add_event_cb(ui_dalle_btn, ui_event_dalle_btn, LV_EVENT_ALL, NULL);

    lv_obj_add_event_cb(ui_api_key_back_btn, ui_event_api_key_back_btn, LV_EVENT_ALL, NULL);

    lv_obj_add_event_cb(ui_chatgpt_back_btn, ui_event_openai_back, LV_EVENT_ALL, NULL);
    lv_obj_add_event_cb(ui_text_edit_gpt_request, ui_event_text_edit_gpt_request, LV_EVENT_ALL, NULL);

    lv_obj_add_event_cb(ui_generate_answer_btn, ui_event_generate_answer_btn, LV_EVENT_ALL, NULL);
    lv_obj_add_event_cb(ui_regennerate_btn, ui_event_regennerate_btn, LV_EVENT_ALL, NULL);
    lv_obj_add_event_cb(ui_chatgpt2_back_btn, ui_event_chatgpt2_back_btn, LV_EVENT_ALL, NULL);

    lv_obj_add_event_cb(ui_dalle_text, ui_event_dalle_text, LV_EVENT_ALL, NULL);
    lv_obj_add_event_cb(ui_dalle_back_btn, ui_event_openai_back, LV_EVENT_ALL, NULL);
    lv_obj_add_event_cb(ui_dalle2_back_btn, ui_event_dalle2_back, LV_EVENT_ALL, NULL);
    lv_obj_add_event_cb(ui_btn_dall_supriseme, ui_event_dall_supriseme, LV_EVENT_ALL, NULL);
    lv_obj_add_event_cb(ui_btn_dall2_regenerate, ui_event_dalle_regennerate, LV_EVENT_ALL, NULL);

    lv_obj_add_event_cb(ui_Keyboard_chatgpt, ui_event_keyboard_hide, LV_EVENT_READY, NULL);
...
static void __view_event_handler(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
    ...
    case VIEW_EVENT_WIFI_ST:
    {
        ...
        lv_img_set_src(ui_wifi_st_12, (void *)p_src);

        if (p_st->is_network)
        {
            openai_run_state.WIFI_CONNECTED = true;
        }
        else
        {
            openai_run_state.WIFI_CONNECTED = false;
        }
        break;
    }
    ...
    case VIEW_EVENT_SCREEN_CTRL:
    {
        ...
        else
        {
            ...
            lv_obj_set_style_bg_color(lv_layer_top(), lv_color_white(), 0);
        }
        break;
    }
    ...
    case VIEW_EVENT_OPENAI_ST:
    {
        ESP_LOGI(TAG, "event: VIEW_EVENT_OPENAI_ST");
        bool *p_st = (bool *)event_data;
        if (*p_st == true)
        {
            openai_run_state.OPENAI_SAVEDKEY = true;
        }
        else
        {
            openai_run_state.OPENAI_SAVEDKEY = false;
        }
        break;
    }

    case VIEW_EVENT_CHATGPT_RESPONSE:
    {
        ESP_LOGI(TAG, "event: VIEW_EVENT_CHATGPT_RESPONSE");
        static struct view_data_openai_response response;
        memset(&response, 0, sizeof(response));
        struct view_data_openai_response *p_data = (struct view_data_openai_response *)event_data;
        memcpy(&response, p_data, sizeof(response));

        lv_label_set_text(ui_text_readonly_gpt_request, lv_textarea_get_text(ui_text_edit_gpt_request));
        if (response.ret)
        {
        ...
        if (ui_request_wait != NULL)
        {
            lv_obj_del(ui_request_wait);
            ui_request_wait = NULL;
        }
        break;
    }
    case VIEW_EVENT_DALLE_RESPONSE:
    {
        ESP_LOGI(TAG, "event: VIEW_EVENT_DALLE_RESPONSE");
        static struct view_data_openai_response response;
        memset(&response, 0, sizeof(response));
        struct view_data_openai_response *p_data = (struct view_data_openai_response *)event_data;
        memcpy(&response, p_data, sizeof(response));
        ...
        if (response.ret)
        {
            ...
            img_png.header.w = 512;
            img_png.header.h = 512;
            img_png.data_size = response.len;
    ...
    case VIEW_EVENT_OPENAI_REQUEST_ST:
    {
        ESP_LOGI(TAG, "event: VIEW_EVENT_OPENAI_REQUEST_ST");
        struct view_data_openai_request_st *p_st = (struct view_data_openai_request_st *)event_data;
        if (ui_request_wait != NULL)
        {
            lv_bar_set_value(ui_request_state_bar, p_st->progress, LV_ANIM_ON);
            lv_label_set_text(ui_request_state_msg, p_st->state);
        }
        break;
    }
    default:
